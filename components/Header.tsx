...
export default function Header() {
  ...
  const currentPage = menuItems.find((item) => item.link === pathname)?.name ?? 'Directory';

  return (
    <>
      <Head>
        <title>{`${currentPage} | bgp.cheap`}</title>
        <meta
          name="description"
          content="A neutral directory of providers offering BGP services."
        />
